...
// for bloom, do a horizonal and a vertical
// 9x9 with downscale sample
...
// 9x9 without downscale sample
float4 BloomVerticalPassFragment(Varyings input) : SV_TARGET {
    float3 color = 0.0;
    // use a trick to apply bilinear filtering to sample this 9 point, down to 5
    float offsets[] = {
...
float4 BloomAddPassFragment(Varyings input) : SV_TARGET {
    ...
    if (_BloomBicubicUpsampling) {
        lowRes = GetSourceBicubic(input.screenUV).rgb; // bicubic upsample
    }
    else {
...
float3 ColorGrade(float3 color, bool useACES = false) {
    // color = min(color, 60.0); // covered in LUT
